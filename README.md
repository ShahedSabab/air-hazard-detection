...
This project utilizes ML algorithms to detect air quality conditions in part of the United States based on satellite data. A Tableau dashboard is developed to visualize the predicted Air Quality Index (AQI) in different regions. The visualization also includes ancillary data to access the impact of the detection. For interactive visualizations [visit this link](https://public.tableau.com/profile/shahed.anzarus.sabab#!/vizhome/AirHazardDetection/StoryofAirHazard?publish=yes).
